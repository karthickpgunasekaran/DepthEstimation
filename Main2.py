...
def EstimatePose(model,depth_enc):
    dof, trans, rotation = model(depth_enc[0:len(depth_enc[4])-2], depth_enc[1:len(depth_enc[4])-1], depth_enc[2:len(depth_enc[4])])
...
def InitModels():
    ...
    #Pose model
    pose_model = pose_model = PoseCopy.PoseExpNet().double()
...
def trianNetworks(img,pose_model,model_enc,model_depth_dec,train_val,no_of_epoch,train_batch,val_batch,iter,camera_intrinsics,log,ssim_fn,optimizer):
    ...
    # Pose estimation
    dof, trans, rot = EstimatePose(pose_model, t)
...
def saveModels(model,name):
    torch.save(model.state_dict(),"Model_m2/"+name)

def createLogFile():
    return open("log_m2.txt","w+")
