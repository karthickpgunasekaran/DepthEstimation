...
def InitModels():
    ...
    #Pose model
    pose_model = PoseNetwork.PoseDecoder().double()
    ...
    if torch.cuda.device_count() > 1:
        ...
        model_depth_dec = nn.DataParallel(model_depth_dec)
...
def trianNetworks(img,pose_model,model_enc,model_depth_dec,train_val,no_of_epoch,train_batch,val_batch,iter,camera_intrinsics,log,ssim_fn,optimizer):
    ...
    print("shape:s",img[0].shape)
    t = np.zeros(shape=(b, 3, img_height, img_width),
                dtype=np.double)
    t = np.transpose(np.asarray(img), (0, 3,1,2))

    print(t.shape)
    print(t)

    t = torch.from_numpy(t,type=torch.double).to(device)
    print(t)
    return
