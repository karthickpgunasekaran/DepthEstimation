...
class PoseExpNet(nn.Module):
    ...
    def forward(self, target_image, ref_img1,ref_img2):
        #print("Pose target image:", target_image.shape)
        #print(" ref imgs:",ref_imgs[0].shape," no of lists:",len(ref_imgs))

        input = torch.cat((target_image,ref_img1,ref_img2), dim=1)
        #assert(len(ref_imgs) == self.nb_ref_imgs)
        #input = [target_image]
        #input.extend(ref_imgs)
        #input = torch.cat(input, 1)
        #print("Pose input shape:",input.shape)
